...
class StFmsDbMaker : public StMaker {
 public: 
  ...
  float getTimeDepCorr(int event, int det, int ch); //det = detectorId - 8 (0=largeNoth, 1=largeSouth, 2=smallNorth, 3=smallSouth)
                                                    //ch=1 to 578
  void maskChannels(Bool_t isHotCh[][571]) {memcpy(mIsHotCh,  isHotCh, sizeof(mIsHotCh)); mManuallyMaskCh = kTRUE;}  //! Latif
 ...
 private:
  ...
  Bool_t                 mIsHotCh[4][571];                  //! Latif
  Bool_t                 mManuallyMaskCh = kFALSE;          //! Latif          
