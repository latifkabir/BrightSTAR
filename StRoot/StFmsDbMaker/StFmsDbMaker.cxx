...
Int_t StFmsDbMaker::InitRun(Int_t runNumber) {
  LOG_INFO << "==================> Loading Local Version of StFmsDbMaker <================ "<< endm;  
  ...
  //------------ Mask hot channels manually ----------------------
  if(mManuallyMaskCh)
  {
      LOG_INFO << "StFmsDbMaker::InitRun - Manually Masking Hot/Bad Channels"<<endm;
      LOG_INFO << "Masked Fms channels:"<<endm;
      Int_t nMaskedCh = 0;
      for(Int_t i = 0; i < 4; ++i)
      {
  	  Int_t MaxCh;
  	  ...
  	  for (Int_t l = 0; l < MaxCh; l++) 
  	  {
  	      if(mIsHotCh[i][l])
	      {
		  LOG_INFO << "det "<< (i + 8) << " ch: "<< (l + 1)<<endm;  
  		  mmGainCorrection[i+8][l].corr = 0.0;
		  ++nMaskedCh;
	      }
  	  }   
      }
      LOG_INFO << "Total Fms channels Masked:"<< nMaskedCh <<endm;
  }
  //-------------------- End of local change -------------------------
