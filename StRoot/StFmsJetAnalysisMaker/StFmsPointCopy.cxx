...
StFmsPointCopy::StFmsPointCopy()
: mDetectorId(0), mEnergy(-1.0), mX(-99.0), mY(-99.0),
mId(-1), mParentClusterId(-1), mNParentClusterPhotons(-1)
{
    resetFps();
}

StFmsPointCopy::~StFmsPointCopy() { /* no op */ }

int   StFmsPointCopy::fpsNCandidate(int layer) {
...
float StFmsPointCopy::fpsMip(int layer, int candidate) {
...
int StFmsPointCopy::fpsSlatId(int layer, int candidate) {
...
float StFmsPointCopy::fpsDistance(int layer, int candidate) {
    if (layer>=1 && layer<=kFpsNLayer && candidate>=0 && candidate<mFpsNCandidate[layer-1]){
...
void StFmsPointCopy::setFps(int layer, float mip, int slatid, float d) {
    if (layer>=1 && layer<=kFpsNLayer){
...
void StFmsPointCopy::orderFpsCandidates(int layer) {  //order candidates by distance
...
void StFmsPointCopy::print(int opt) {
