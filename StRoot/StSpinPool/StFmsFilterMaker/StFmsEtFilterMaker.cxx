//*-- Author : Jan Balewski
// 
// $Id: StFmsEtFilterMaker.cxx,v 1.1 2018/05/22 19:55:42 akio Exp $
...
#include "StFmsEtFilterMaker.h"

ClassImp(StFmsEtFilterMaker)

//_____________________________________________________________
//_____________________________________________________________
StFmsEtFilterMaker::StFmsEtFilterMaker(const char *name):StMaker(name){
  par_E_thres= 30;
  par_Z0_vert=-9999;
  par_delZ_vert=0.;
  //  myMode=kUnknown;
...
//_____________________________________________________________
/// This is  destructor
StFmsEtFilterMaker::~StFmsEtFilterMaker(){
...
//_____________________________________________________________
//_____________________________________________________________
Int_t StFmsEtFilterMaker::FinishRun(int runumber){
...
/*
Changes made wrt CVS version (StFmsFilterMaker):
- Use transverse energy sum instead of energy sum
- Remove top and bottom pie areas which do not exist in real trigger. This reduces 8 phi bins to 6 phi bins (quadrants).
- Fix bug for right pie phi bin
*/
//_____________________________________________________________
Int_t StFmsEtFilterMaker::triggerCondition()
{  
  ...
  Float_t energyQuadrant[6] = {0,0,0,0,0,0};
  Int_t binPhi = 6;
  for(int i=0; i<nhits; i++){
    ...
    if(dId>7&&dId<12 && energy>0.2){
      ...
      Float_t phi =  v3.phi();
      Float_t theta =  v3.theta();
      
      binPhi = 6;

      if((phi<   0*(M_pi/4))    && (phi > -2.0*(M_pi/4))) binPhi = 0;  //lower left quadrant wrt beam
      if((phi< -2.0*(M_pi/4))   && (phi > -4.0*(M_pi/4))) binPhi = 1;  //lower right
      if((phi <  4.0*(M_pi/4))  && (phi > 2.0*(M_pi/4))) binPhi = 2;   //upper right
      if((phi <  2.0*(M_pi/4))  && (phi >   0*(M_pi/4))) binPhi = 3;   //upper left

      if((phi< 1.0*(M_pi/4))    && (phi > -1.0*(M_pi/4))) binPhi = 4;  //left pie wrt beam 
      if((phi < -3.0*(M_pi/4)  && phi >= -4.0*(M_pi/4)) || (phi > 3.0*(M_pi/4)  && phi <= 4.0*(M_pi/4))) binPhi = 5;  // right pie
      
      if(binPhi<6)   energyQuadrant[binPhi] += energy*fabs(sin(theta));

    }
  }
  Int_t iOrd[6];
  TMath::Sort(6,  energyQuadrant, iOrd);
...
//_____________________________________________________________
//_____________________________________________________________
Float_t StFmsEtFilterMaker::transverseNRG(Float_t vertexPosZ, StFmsTower *tower)
{
