...
//-------------------------------------
void StFmsCalibMaker::ResetLeaves(void)
{
	mHit = 0;
...
//-------------------------------------------------------
void StFmsCalibMaker::SetReadBadChannel(const char* list)
{
	mReadBadCh = true;
	...
	if (!in.is_open())
	{
		LOG_ERROR <<"StFmsCalibMaker::ReadBadCh - cannot open the bad channel list!\n" <<endm;
		return;
	}
	while (in.is_open())
	{
...
//------------------------------------------------------------
Int_t StFmsCalibMaker::CheckFmsTrig(const StTriggerId& trigId)
{
    int trigFired = 0;
...
//===================================================================
StFmsCalibMaker::StFmsCalibMaker(const char* name) : StMaker(name) {}

//===========================
Int_t StFmsCalibMaker::Init()
{
	mFmsDbMk = static_cast<StFmsDbMaker*>(GetMaker("fmsDb"));
	if (!mFmsDbMk) { LOG_ERROR <<"StFmsCalibMaker::InitRun - !StFmsDbMaker" <<endl; return kStFatal; }
...
Int_t StFmsCalibMaker::InitRun(int runNo)
{
	...
	if (mGetQaTree)
	{
		...
		int iLeaf = 0;
		mTree = new TTree("T", "FMS calibration QA");
...
Int_t StFmsCalibMaker::Make()
{
    ...
    if (!muDST) { LOG_ERROR <<"StFmsCalibMaker::Make - !MuDst" <<endl; return kStErr; }
	else
	{
	...
	if (!event) { LOG_ERROR <<"StFmsCalibMaker::Make - !StEvent" <<endl; return kStErr; }
