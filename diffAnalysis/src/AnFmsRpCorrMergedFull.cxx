...
void AnFmsRpCorrMergedFull()
{
    TString filePath = "/star/u/kabir/GIT/BrightSTAR/dst/R15MergedFull/R15FmsRpTree_Merged.root";
    ...
    TH1D *hist9 = new TH1D("sumE_west", "E_{p + #pi^{0}}^{West}; E_{p + #pi^{0}}^{West} [GeV]", 200, 60, 200);
	...
	for(Int_t pair = 0; pair < reader->fms_nPairs; ++pair)
	{
	    ...
	    fms_i = pair;
	    break; //consider only highest energy pair of photons, note: the Pions are already sorted based on energy
	}
