...
// the main routine
//
void AnRunEEmcRpTreeMakerPart1(const Char_t *inputFileName,
			    ...
			    Bool_t isMC)
{
    ...
    vector<Int_t> trigs(1);
    //------- For FMS stream ----------------
    string fileList = inputFileName;
    Int_t runNumber = TStRunList::GetRunFromFileName(fileList);
    if(runNumber < 1)
    {
    	cout << "Unable to get run number" <<endl;
    	return;
    }
    trigs[0] = TStTrigDef::GetTrigId(runNumber, "EHT0"); // Generally 480301;
    ...
    filterMaker->SetHist1d(rpTreeMaker->GetEvtCountHist());
    //
    ...
    for( ; nevents <= neventsIn && treeMakerPtr->getNumPart1EventsWritten() < neventsOut; ++nevents ){
	// clear
	analysisChain->Clear();
