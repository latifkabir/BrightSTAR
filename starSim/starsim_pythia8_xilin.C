...
StarPrimaryMaker *sim_primary = 0;
...
void Pythia6( TString mode="pp:W", Int_t tune=320 )
{
  ...
  sim_primary->AddGenerator(pythia6);
}
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
void Pythia8( TString config="pp:W", const char* _library="libPythia8_1_62.so" )
{
  ...
  sim_primary -> AddGenerator( pythia8 );
  
}
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
void starsim_pythia8_xilin( Int_t nevents=1000, UInt_t rngSeed = 12345 )
{ 
...
//    TString simple = "tables nodefault";
  ...
  //
  // Create the sim_primary event generator and insert it
  // before the geant maker
  //
  //  StarPrimaryMaker *
  sim_primary = new StarPrimaryMaker();
  {
    sim_primary -> SetFileName( Form("pythia8_%i.starsim.root",rngSeed));
      chain -> AddBefore( "geant", sim_primary );
  }
  ...
  //   set ptmin=1.0E9.)
  //                    ptmin  ptmax
  sim_primary->SetPtRange  (0.0,  -100.0);         // GeV
  //                    etamin etamax
  //sim_primary->SetEtaRange ( 2.5, 4.0 );
  //                    phimin phimax
  sim_primary->SetPhiRange ( 0., TMath::TwoPi() );
  
  
  // 
  // Fixed x, y, z vertex
  // 
  sim_primary->SetVertex( 0., 0., 0. );
  sim_primary->SetSigma( 0., 0.1, 0. );

  //
  // Initialize sim_primary event generator and all sub makers
  //
  sim_primary -> Init();
  ...
  geometry("y2015a");
  command("gkine -4 0");
  command(Form("gfile o FmsSim_Run15_%i_evt%i.fzd",rngSeed,nevents));
...
// ----------------------------------------------------------------------------
  StarGenEvent* event = sim_primary->event();
