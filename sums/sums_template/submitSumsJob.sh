...
export OUTNAME=$3
export JOBNAME=$4
export SUMSDIR=$STARHOME/sums/sums_template
...
do
    ...
    star-submit-template \
    	-template $SUMSDIR/scheduler_template.xml \
    	-entities STARHOME=$STARHOME,MACRO=$MACRO,OUTDIR=$OUTDIR,JOBOUTDIR=$JOBOUTDIR,OUTNAME=$OUTNAME,RUN=$RUN,SOURCE=$SOURCE,ROOTSETUP=$ROOTSETUP,FILEN=$FILEN,FILET=$FILET,PROD=$PROD,TRIG=$TRIG,SETUPFILE=$SETUPFILE,LIBBRIGHT=$LIBBRIGHT,CONFIG=$CONFIG,DATABASE=$DATABASE,MAXFILES=$MAXFILES 
