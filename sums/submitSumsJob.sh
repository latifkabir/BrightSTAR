...
do
    ...
    star-submit-template \
    	...
    	-entities STARHOME=$STARHOME,MACRO=$MACRO,OUTDIR=$OUTDIR,JOBOUTDIR=$JOBOUTDIR,OUTNAME=$OUTNAME,RUN=$RUN,SOURCE=$SOURCE,ROOTSETUP=$ROOTSETUP,FILEN=$FILEN,FILET=$FILET,PROD=$PROD,TRIG=$TRIG,SETUPFILE=$SETUPFILE,LIBBRIGHT=$LIBBRIGHT,LIBANA=$LIBANA,LIBMACROS=$LIBMACROS,LIBDIFFANA=$LIBDIFFANA,MAXFILES=$MAXFILES 
