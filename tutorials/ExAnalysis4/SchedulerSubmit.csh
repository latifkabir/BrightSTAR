...
foreach RUN (`cat $RUNLIST`)
	...
	mkdir -p ./log_$OUTNAME/$RUN
	mkdir -p ./out_$OUTNAME
	mkdir -p ./report
	...
	star-submit-template \
	 ...
	 -entities BASEDIR=$BASEDIR,MACRO=$MACRO,OUTNAME=$OUTNAME,RUN=$RUN,SOURCE=$SOURCE,FILEN=$FILEN,FILET=$FILET,PROD=$PROD,TRIG=$TRIG
