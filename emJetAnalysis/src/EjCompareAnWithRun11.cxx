...
void EjCompareAnWithRun11(TString fileName)
{
	...
	for(Int_t j = 0; j < kPhotonBins; ++j)
	{
	    ...
	    if(i == 0)
	    	yGrPhy[i][j] = new TGraphErrors(8, x11e1, y11e1[j], 0, 0);
	    if(i == 1)
	    	yGrPhy[i][j] = new TGraphErrors(9, x11e2, y11e1[j], 0, 0);
	    if(i == 2)
	    	yGrPhy[i][j] = new TGraphErrors(7, x11e3, y11e1[j], 0, 0);
	...
	for(Int_t j = 0; j < 3; ++j)
	{
	    ...
	    yGrPhy[j][i]->SetMarkerStyle(kOpenCircle);
   ...
   for (Int_t j=0;j<Ny;j++)
   {
	   ...
	   yGrPhy[i][4 - j]->SetMarkerStyle(kOpenCircle);
	   
	   bGrPhy[i+2][4 - j]->SetMaximum(0.05);
	   bGrPhy[i+2][4 - j]->SetMinimum(-0.05);

	   yGrPhy[i][4 - j]->SetMaximum(0.05);
	   yGrPhy[i][4 - j]->SetMinimum(-0.05);
