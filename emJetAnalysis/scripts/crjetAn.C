...
//using cross-ratio method with sin(phi) fit
  Int_t crjetAn( Float_t elow = 48, Float_t ehigh = 53, Float_t mlow = 1.00, Float_t mhigh = 1.00, Int_t fillnum = 18790,char* infile="/star/u/zwzhu/01zwzhu/Run15/byfill/ipi0jetcor_stfms_Fill18790.root" ){
...
//        }
	...
	sprintf(resultname,"jetAnJProf_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV.txt",fillnum,mlow,mhigh,elow,ehigh);
	sprintf(resultname2,"jetAnyJProf_Fill%d_mass%.2fto%.2fGeV_E%.1fto%.1fGeV.txt",fillnum,mlow,mhigh,elow,ehigh);
	...
	Int_t JP=((0x07)<<8); // Corrected
	//Int_t JP=((0x07)<<7); // BUG!!!!!!!!!!! This should be (0x7) << 8 based on DST generation or above convention
	Int_t HiTow=(0x03);
	Int_t clbit=((0x03F)<<2);

	Int_t TrigBitsSelect = JP;
        Char_t Rof[50];
	//sprintf(Rof,"bbcm>0.5 && tofm>2.5");
		sprintf(Rof,"1");

//	Int_t TrigBitsSelect = JP+HiTow+clbit;
        Char_t trigselect[400];
        sprintf(trigselect,"(TrigBits&%d&&TMath::Abs((vertexzyuxi+18.4)<93.4))&&fjetntow>0&&%s",TrigBitsSelect,Rof);
        Char_t ptnew[400];
	sprintf(ptnew,"((%.4f)+(%.8f)*fjpT+ TMath::Exp((fjpT-(%.4f))*(%.4f)))*(fjpT-((%.4f)+(%.8f)*fjpT+ TMath::Exp((fjpT-(%.4f))*(%.4f))))",e3,e2,e0,e1,uept3,uept2,uept0,uept1);    // Corrected
        ///////*sprintf(ptnew,"((%.4f)+(%.8f)*fjpT+ TMath::Exp((fjpT-(%.4f))*(%.4f)))*(fjpT-((%.4f)+(%.8f)*fjE+ TMath::Exp((fjE-(%.4f))*(%.4f))))",e3,e2,e0,e1,uept3,uept2,uept0,uept1);*/  //<------------ Initial BUG!!! BUG!!!
